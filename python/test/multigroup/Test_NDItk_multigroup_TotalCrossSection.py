...
class Test_NDItk_multigroup_TotalCrossSection( unittest.TestCase ) :
    ...
    def test_component( self ) :
        ...
        def verify_chunk( self, chunk ) :
            ...
            # verify the record
            self.assertEqual( 'sig_tot', chunk.keyword )
        ...
        chunk = TotalCrossSection( values = [ 0.1, 0.2, 0.25, 0.05, 0.15, 0.04, 0.06 ] )

        verify_chunk( self, chunk )

        # the data is read from a string
        chunk = TotalCrossSection.from_string( self.chunk_string, 7 )
