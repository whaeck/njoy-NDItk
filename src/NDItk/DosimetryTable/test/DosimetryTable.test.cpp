...
SCENARIO( "DosimetryTable" ) {
  ...
  GIVEN( "valid data for a DosimetryTable instance" ) {

    std::string record = chunk();

    WHEN( "the data is given explicitly" ) {

      std::string zaid = "92235.000nd";
      ...
      DosimetryTable chunk( std::move( zaid ), std::move( name ),
                            std::move( process ), awr, temperature, dilution,
                            std::move( structure ), std::move( weights ),
                            std::move( total ), std::move( xs ),
                            std::move( information ), std::move( source ), weight );

      THEN( "a DosimetryTable can be constructed and members can "
            "be tested" ) {
  ...
  GIVEN( "invalid data for a DosimetryTable instance" ) {
    ...
    WHEN( "the number of groups is inconsistent" ) {
      ...
      THEN( "an exception is thrown" ) {
        ...
        CHECK_THROWS( DosimetryTable( std::move( zaid ), std::move( name ),
                                      std::move( process ), awr, temperature, dilution,
                                      std::move( structure ),
                                      {}, std::move( total ), std::move( xs ),
...
void verifyChunk( const DosimetryTable& chunk ) {
  ...
  // metadata
  CHECK( "92235.000nd" == chunk.metadata().zaid() );
  CHECK( "this is some information for the table" == chunk.metadata().information() );
  CHECK( "testing" == chunk.metadata().libraryName() );
